...
存放碰撞图的图片目录。默认为img/parallaxes/collision

**文件名前缀**

碰撞图的文件名前缀，文件名由前缀+地图id组成。默认为map

**每格像素尺寸**

碰撞图中，对应每一格图块的尺寸，必须为偶数。默认为48

**判定颜色**

用于判定为碰撞/无法通行的颜色。默认为红色（#FF0000）
...
防止图片缩放、软边笔刷等情况设置的兜底参数，颜色不透明度较低时也可以识别。正常情况下保持为1即可。
